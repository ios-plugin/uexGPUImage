...
-(void)changeImage:(CustomControl*)btn{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        if (btn.frame.origin.x > KSCREEN_WIDTH/2 && btn.frame.origin.x<self.scrollView.contentSize.width-KSCREEN_WIDTH) {
            
            [self.scrollView setContentOffset:CGPointMake(btn.frame.origin.x-BUTTON_WIDTH, 0) animated:YES];
            
        }
        else if( btn.frame.origin.x>self.scrollView.contentSize.width-KSCREEN_WIDTH &&  btn.frame.origin.x<self.scrollView.contentSize.width){
             [self.scrollView setContentOffset:CGPointMake(self.scrollView.contentSize.width-KSCREEN_WIDTH, 0) animated:YES];
        }else{
...
- (NSString *)getSaveDirPath{
    ...
    return [wgtTempPath stringByAppendingPathComponent:@"uexGPUImage"];
}
